...
export const loadColumns = async (
  ...
  return [
...

    {
      ...
      getOptionValue: (value: any) => value?.id,
      getOptionLabel: (value: any) => value?.label,
      valueOptions: await callOptionsApi('industries'),
      valueGetter: (params: GridValueGetterParams) =>
        params?.value?.id ?? params?.value,
    },
...

    {
      ...
      type: 'singleSelect',
      valueFormatter: ({ value }) =>
        dataFormatter.color_codesManyListFormatter(value).join(', '),
      renderEditCell: (params) => (
        <DataGridMultiSelect {...params} entityName={'color_codes'} />
      ),
    },

    {
      field: 'brand_style',
      headerName: 'BrandStyle',
      flex: 1,
      minWidth: 120,
      filterable: false,
      headerClassName: 'datagrid--header',
      ...
      getOptionValue: (value: any) => value?.id,
      getOptionLabel: (value: any) => value?.label,
      valueOptions: await callOptionsApi('brand_styles'),
      valueGetter: (params: GridValueGetterParams) =>
        params?.value?.id ?? params?.value,
    },
