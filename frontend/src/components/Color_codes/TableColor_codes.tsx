...
import { loadColumns } from './configureColor_codesCols';
...
const TableSampleColor_codes = ({
  ...
  const {
    ...
    loading,
    count,
    notify: color_codesNotify,
    refetch,
  } = useAppSelector((state) => state.color_codes);
  ...
  useEffect(() => {
    if (color_codesNotify.showNotification) {
      notify(
        color_codesNotify.typeNotification,
        color_codesNotify.textNotification,
      );
    }
  }, [color_codesNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/color_codes/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/color_codes/color_codes-view/?id=${id}`);
  };
...
export default TableSampleColor_codes;
